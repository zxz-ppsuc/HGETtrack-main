...
from lib.test.tracker.data_utils import Preprocessor, PreprocessorHSI
from lib.utils.box_ops import clip_box
from lib.utils.ce_utils import generate_mask_cond


class OSTrack(BaseTracker):
    def __init__(self, params): #, dataset_name 由于代码调整，去掉这一参数
        super(OSTrack, self).__init__(params)
        network = build_ostrack(params.cfg, training=False)
        network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True) #推理加载别人模型
        self.cfg = params.cfg
